...
function generateChallengePage(slug: string, delayMs: number, functionUrl: string): string {
  // Use encodeURIComponent to safely embed values
  const safeSlug = slug.replace(/'/g, "\\'").replace(/"/g, '\\"');
  const safeFunctionUrl = functionUrl.replace(/'/g, "\\'").replace(/"/g, '\\"');
...
var postUrl = '${safeFunctionUrl}/${safeSlug}';
...
mouseMovements:events.mouse,
scrollEvents:events.scroll,
keypressEvents:events.key,
touchEvents:events.touch,
focusChanges:events.focus,
...
if(d){fp.webglVendor=gl.getParameter(d.UNMASKED_VENDOR_WEBGL);fp.webglRenderer=gl.getParameter(d.UNMASKED_RENDERER_WEBGL);}
}
}catch(e){}
...
fp.canvasHash=c2.toDataURL().slice(-50);
}catch(e){}

try{
fp.isHeadless=/HeadlessChrome/.test(navigator.userAgent)||navigator.webdriver===true||!window.chrome||window.outerWidth===0||window.outerHeight===0;
fp.isAutomated=navigator.webdriver||!!window.callPhantom||!!window._phantom||!!window.__nightmare||!!document.__selenium_unwrapped;
}catch(e){}
...
setTimeout(function(){
var data={slug:'${safeSlug}',fingerprint:collectFingerprint()};
