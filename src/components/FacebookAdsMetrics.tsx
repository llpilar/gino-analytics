...
export const FacebookAdsMetrics = () => {
  ...
  const { data: accountsData, isLoading: accountsLoading, error: accountsError } = useFacebookAdAccounts();
  const { data: insightsData, isLoading: insightsLoading } = useFacebookAdInsights(selectedAccount, startDate, endDate);

  // Check if needs connection
  if (accountsData?.needsConnection || accountsError) {
    return (
      <Alert className="border-blue-500/20 bg-blue-500/10">
        <Facebook className="h-4 w-4 text-blue-400" />
        <AlertDescription className="text-zinc-300">
          Conecte sua conta do Facebook para visualizar as métricas de anúncios.
          <br />
          <Button 
            variant="link" 
            className="text-blue-400 hover:underline p-0 h-auto mt-2"
            onClick={() => navigate('/configuracoes')}
          >
            Conectar Facebook nas Configurações →
          </Button>
        </AlertDescription>
      </Alert>
    );
